// const { validateEmployeeUser } = require("../services/authService");
// const bcrypt = require("bcrypt");

// // Import User model and Google Auth Strategy
// const User = require("../models/User");

// const salt = 10;

// // Retrieve user data
// const getUserData = async (id, done) => {
//   try {
//     const user = await User.findById(id);
//     done(null, user);
//   } catch (error) {
//     console.error({ message: error.message });
//     done(null, false);
//   }
// };

// // Create new user if not registered
// // TODO - Rewrite register user method to find local user
// // and to encrypt password locally

// const registerLocalUser = async (req, res) => {
//   const { email, password: plainTextPassword } = req.body;

//   try {
//     const password = await bcrypt.hash(plainTextPassword, salt);
//     const response = await User.create({
//       email,
//       password,
//     });
//     res.send(response);
//   } catch (error) {
//     console.error({ message: error.message });
//     if (error.code === 11000)
//       return res.send({ status: "error", error: "email already exists" });
//     throw error;
//   }
// };

// const registerUser = async (profile, done) => {
//   const newUser = {
//     googleId: profile.id,
//     displayName: profile.displayName,
//     firstName: profile.name.givenName,
//     lastName: profile.name.familyName,
//     profileImage: profile.photos[0].value,
//     email: profile.emails[0].value,
//   };

//   try {
//     let user = await User.findOne({ googleId: profile.id });

//     if (user) {
//       done(null, user);
//     } else {
//       if (!validateEmployeeUser(newUser)) {
//         done(null, false);
//       } else {
//         // create user if user email has employee address
//         user = await User.create(newUser);
//         // validate user email is an employee address
//         done(null, user);
//       }
//     }
//   } catch (error) {
//     done(error, null);
//   }
// };

// const logOutUser = (req, res) => {
//   req.session.destroy((err) => {
//     if (err) {
//       console.error(err);
//       res.send("Error: user logout failed...");
//     } else {
//       res.redirect("/");
//     }
//   });
// };

// module.exports = {
//   getUserData,
//   registerUser,
//   logOutUser,
//   registerLocalUser,
// };

const bcrypt = require("bcrypt");
const User = require("../models/User");
const { verifyUserLogin } = require("../services/authService");

// set constants for authentication
const salt = 10;

// Sign up new user and hash password set
const signUpUser = async (req, res) => {
  // getting our data from frontend
  const { email, password: plainTextPassword } = req.body;

  // encrypting our password to store in database
  const password = await bcrypt.hash(plainTextPassword, salt);
  try {
    // storing our user data into database
    const response = await User.create({
      email,
      password,
    });
    console.log(response);
    return res.redirect("/");
  } catch (error) {
    console.log(JSON.stringify(error));
    if (error.code === 11000) {
      return res.send({ status: "error", error: "email already exists" });
    }
    throw error;
  }
};

// login user and verify user with json web token
const loginUser = async (req, res) => {
  const { email, password } = req.body;
  // we made a function to verify our user login
  const response = await verifyUserLogin(email, password);
  console.log(req.cookies );
  if (response.status === "ok") {
    // storing our JWT web token as a cookie in our browser
    res.cookie("token", token, { maxAge: 2 * 60 * 60 * 1000, httpOnly: true }); // maxAge: 2 hours
    res.redirect("/");
  } else {
    res.json(response);
  }
};

module.exports = {
  signUpUser,
  loginUser,
};
